...
class _MandiScreenState extends State<MandiScreen> {
  ...
  List<String> get _availableDistricts {
    if (_selectedState.isEmpty || !MandiService.indianStatesDistricts.containsKey(_selectedState)) {
      return [];
    }
    final list = List<String>.from(MandiService.indianStatesDistricts[_selectedState]!);
    list.sort();
    return list;
  }
  ...
  Future<void> _fetchMandiPrices() async {
    ...
    if (_selectedState.isEmpty || _selectedDistrict.isEmpty || _selectedCrop.isEmpty) {
      setState(() {
        _errorMessage = 'Please select State, District, and Crop before searching.';
      });
      return;
    }
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _errorMessage = 'Error querying Mandi API. Please check internet connection.';
          _isLoading = false;
        });
      }
    }
                        ...
                        child: Row(
                          children: [
                            ...
                            Expanded(
                              child: Text(
                                _errorMessage!,
                                style: GoogleFonts.poppins(color: Colors.red, fontSize: 12.5, fontWeight: FontWeight.w600),
                              ),
                            ),
                          ],
                        ),
                    ...
                    SliverToBoxAdapter(
                      child: Padding(
                        ...
                        child: GlassCard(
                          ...
                          child: Row(
                            children: [
                              const Icon(Icons.info_outline_rounded, color: AppColors.accentGold, size: 22),
                              const SizedBox(width: 10),
                              Expanded(
                                child: Text(
                                  'No active Mandi price record found for "$_selectedCrop" in $_selectedDistrict, $_selectedState today.',
                                  style: GoogleFonts.poppins(color: AppColors.textPrimary, fontSize: 12.5, fontWeight: FontWeight.w600),
                                ),
                              ),
                            ],
                          ),
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        'Mandi Prices',
                        style: GoogleFonts.poppins(fontSize: 19, fontWeight: FontWeight.w800, color: AppColors.textPrimary),
                        maxLines: 1,
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                ...
                Text(
                  'OGD LIVE',
                  style: GoogleFonts.poppins(fontSize: 10, fontWeight: FontWeight.w800, color: AppColors.leafGreen),
                ),
              ],
            ),
          ),
    ...
    return GlassCard(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              const Icon(Icons.storefront_rounded, color: AppColors.leafGreen, size: 22),
              const SizedBox(width: 8),
              Expanded(
                child: Text(
                  'Mandi Market Intelligence',
                  style: GoogleFonts.poppins(fontSize: 15.5, fontWeight: FontWeight.w800, color: AppColors.textPrimary),
                  maxLines: 1,
                  overflow: TextOverflow.ellipsis,
                ),
              ),
          ...
          Text(
            ...
            style: GoogleFonts.poppins(fontSize: 11.5, color: AppColors.textMuted, height: 1.35),
          ),
          ...
          MandiDropdownSelector(
            ...
            placeholder: _selectedState.isEmpty ? 'Select State First' : 'Select District',
            onChanged: (dt) {
                  ...
                  : Row(
                      ...
                      children: [
                        ...
                        Text(
                          ...
                          style: GoogleFonts.poppins(fontSize: 14.5, fontWeight: FontWeight.w800, color: Colors.white),
                        ),
                      ],
                    ),
            ),
  ...
  Widget _buildLocalSectionHeader() {
    ...
    return Row(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Icon(Icons.location_city_rounded, color: AppColors.leafGreen, size: 20),
        const SizedBox(width: 8),
        Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              ...
              Text(
                scopeNote,
                style: GoogleFonts.poppins(fontSize: 11, fontWeight: FontWeight.w600, color: AppColors.leafGreen),
                maxLines: 2,
                overflow: TextOverflow.ellipsis,
              ),
            ],
  ...
  Widget _buildMandiCard(MandiPriceRecord mandi) {
    return Container(
      decoration: BoxDecoration(
        border: const Border(left: BorderSide(color: AppColors.leafGreen, width: 4)),
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Icon(Icons.store_rounded, color: AppColors.leafGreen, size: 19),
                const SizedBox(width: 6),
                Expanded(
                  child: Text(
                    '${mandi.market} Mandi',
                    style: GoogleFonts.poppins(fontSize: 14.5, fontWeight: FontWeight.w800, color: AppColors.textPrimary),
                    maxLines: 2,
                    overflow: TextOverflow.ellipsis,
                  ),
                ),
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(color: AppColors.leafGreen.withValues(alpha: 0.25)),
                  ),
                  child: Text(
                    '📍 ${mandi.district}, ${mandi.state}',
                    style: GoogleFonts.poppins(fontSize: 10, fontWeight: FontWeight.w700, color: AppColors.leafGreen),
                    maxLines: 1,
                    overflow: TextOverflow.ellipsis,
                  ),
                ),
            ...
            Container(
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(12),
                border: Border.all(color: AppColors.leafGreen.withValues(alpha: 0.15)),
              ),
              child: Row(
                children: [
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('Modal Price (Quintal)', style: GoogleFonts.poppins(fontSize: 10, color: AppColors.textMuted, fontWeight: FontWeight.w600)),
                        const SizedBox(height: 2),
                        FittedBox(
                          fit: BoxFit.scaleDown,
                          child: Text(
                            '₹${mandi.modalPriceQuintal.toStringAsFixed(0)}',
                            style: GoogleFonts.poppins(fontSize: 19, fontWeight: FontWeight.w800, color: AppColors.leafGreen),
                          ),
                        ),
                        Text('per Quintal (100 kg)', style: GoogleFonts.poppins(fontSize: 9.5, color: AppColors.textMuted)),
                      ],
                    ),
                  ),
                  Container(width: 1, height: 38, color: AppColors.leafGreen.withValues(alpha: 0.2)),
                  const SizedBox(width: 10),
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('Converted Price (Kg)', style: GoogleFonts.poppins(fontSize: 10, color: AppColors.textMuted, fontWeight: FontWeight.w600)),
                        const SizedBox(height: 2),
                        FittedBox(
                          fit: BoxFit.scaleDown,
                          child: Text(
                        ...
                        Text('per Kilogram', style: GoogleFonts.poppins(fontSize: 9.5, color: AppColors.textMuted)),
                      ],
                    ),
                  ),
                ],
            ...
            Row(
              children: [
                Expanded(child: _buildMetaItem('Min Price', '₹${mandi.minPriceQuintal.toStringAsFixed(0)}/qtn (₹${mandi.minPriceKg.toStringAsFixed(1)}/kg)')),
                Expanded(child: _buildMetaItem('Max Price', '₹${mandi.maxPriceQuintal.toStringAsFixed(0)}/qtn (₹${mandi.maxPriceKg.toStringAsFixed(1)}/kg)')),
              ],
            ),
            const SizedBox(height: 8),
            Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(color: AppColors.leafGreen.withValues(alpha: 0.18)),
              ),
              child: Row(
                children: [
                  const Icon(Icons.calendar_month_rounded, size: 14, color: AppColors.leafGreen),
                  const SizedBox(width: 6),
                  Text(
                    'Arrival Date: ',
                  ...
                  Expanded(
                    child: Text(
                      ...
                      style: GoogleFonts.poppins(fontSize: 11.5, fontWeight: FontWeight.w800, color: AppColors.textPrimary),
                    ),
                  ),
                ],
              ),
  ...
  // ─── SECTION 2: Highest Mandi Price in India ──────────────────────────────
  Widget _buildHighestMandiCard(MandiHighestRecord highest, double? diffPercent, double? diffAmount) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
          ...
          children: [
            ...
            Expanded(
              child: Text(
                ...
                style: GoogleFonts.poppins(fontSize: 15.5, fontWeight: FontWeight.w800, color: AppColors.accentGold),
                maxLines: 1,
                overflow: TextOverflow.ellipsis,
              ),
            ),
              ...
              children: [
                  ...
                  children: [
                      ...
                      child: Column(
                        ...
                        children: [
                          Text(
                            ...
                            style: GoogleFonts.poppins(fontSize: 9.5, fontWeight: FontWeight.w800, color: AppColors.accentGold, letterSpacing: 0.5),
                          ),
                          const SizedBox(height: 2),
                          Text(
                    ...
                    if (diffPercent != null && diffPercent > 0)
                      Container(
                        ...
                        child: Text(
                          '⚡ +$diffPercent% Higher',
                          style: GoogleFonts.poppins(fontSize: 10.5, fontWeight: FontWeight.w800, color: AppColors.accentGold),
                        ),
                      ),
                  ],
                ),
                    ...
                    children: [
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text('Highest Mandi Price', style: GoogleFonts.poppins(fontSize: 10, color: AppColors.textMuted, fontWeight: FontWeight.w600)),
                            const SizedBox(height: 2),
                            FittedBox(
                              fit: BoxFit.scaleDown,
                              child: Text(
                                '₹${highest.maxPriceQuintal.toStringAsFixed(0)}',
                                style: GoogleFonts.poppins(fontSize: 20, fontWeight: FontWeight.w800, color: AppColors.accentGold),
                              ),
                            ),
                            Text('per Quintal (100 kg)', style: GoogleFonts.poppins(fontSize: 9.5, color: AppColors.textMuted)),
                          ],
                      ...
                      Container(width: 1, height: 38, color: AppColors.accentGold.withValues(alpha: 0.25)),
                      const SizedBox(width: 10),
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                ...
                Row(
                  children: [
                    ...
                    Expanded(child: _buildMetaItem('Modal Price', '₹${highest.modalPriceQuintal.toStringAsFixed(0)}/qtn')),
                  ],
                ),
                const SizedBox(height: 8),
                Container(
                  ...
                  child: Row(
                    children: [
                      const Icon(Icons.calendar_month_rounded, size: 14, color: AppColors.accentGold),
                      const SizedBox(width: 6),
                      Text(
                        'Arrival Date: ',
                        style: GoogleFonts.poppins(fontSize: 11, fontWeight: FontWeight.w600, color: AppColors.textMuted),
  ...
  Widget _buildMetaItem(String label, String value) {
    return Column(
      ...
      children: [
        Text(
          label,
          style: GoogleFonts.poppins(fontSize: 9.5, color: AppColors.textMuted, fontWeight: FontWeight.w600),
          maxLines: 1,
          overflow: TextOverflow.ellipsis,
        ),
        Text(
